...
import {FETCH_PROFILES_QUERY} from '../util/graphql'
...
function Home() {
  ...
  useEffect(() => {
	...
	useEffect(() => {
		if(data && (trucks.length == 0 || locations.length == 0)){
			const dataObj = data.getProfiles;
			dataObj.map((p) => {
				
				const fetched_trucks = p.trucks;
				fetched_trucks.forEach((t) => {
					const [locationObj] = t.location;
					if (locationObj && t.status) {
						setLocation((prevLocation) => [...prevLocation, { ...locationObj, avatar: p.avatar || "https://res.cloudinary.com/rushed21/image/upload/v1621689254/foodtruck/avatars/avatar.png" }]);
						setTrucks(trucks => [...trucks, t]);
					}
				});
			});
		}
		...
		markerState && markerProfile ? (
			<div className="absolute bottom-0 md:bottom-25 md:left-5 w-screen md:w-1/3 xl:w-96  p-2 sm:px-32 md:p-4 bg-coral z-30 rounded-3xl">
				<div className="flex justify-center -mt-12">
					{" "}
					<img src={markerProfile.avatar || "https://res.cloudinary.com/rushed21/image/upload/v1621689254/foodtruck/avatars/avatar.png"} className=" h-24 w-24 rounded-3xl" alt={markerProfile.title} />{" "}
				</div>
				...
				<div className="flex justify-center px-2 pt-4">
					<motion.p
						className="bg-night text-white font-mulish font-normal text-center text-base w-24 py-2 mx-1 my-2 rounded-xl z-40"
					...
					<Link to={`/profile/${markerProfile.username}`}>
						<motion.p className="bg-night text-white font-mulish font-normal text-center text-base w-24 py-2 mx-1 my-2 rounded-xl z-40" whileHover={{ scale: 1.1 }} whileFocus={{ scale: 1.2 }} whileTap={{ scale: 0.9 }}>
							Profile
						</motion.p>
					</Link>
					<a href={`https://www.google.com/maps/dir/?api=1&destination=${markerLoc.latitude},${markerLoc.longitude}`}>
						<motion.p className="bg-night text-white font-mulish font-normal text-center text-base w-24 py-2 mx-1 my-2 rounded-xl z-40" whileHover={{ scale: 1.1 }} whileFocus={{ scale: 1.2 }} whileTap={{ scale: 0.9 }}>
							Directions
						</motion.p>
					</a>
...
//////////////// Handle Map Viewports///////////////
  ...
  const getmyLocation = async (e) => {setViewport({
    ...
    return viewport?.latitude ? (
		<div className="top-0 left-0 z-0">
			<div className="h-screen w-screen">
				...
				<Suspense
					fallback={
						<div className="absolute flex h-screen w-screen justify-center items-center">
							<h1 className="font-poppins font-bold mb-16 text-lg md:text-4xl">
								Map is Loading. <br />
								Your en-route to great food...
							</h1>
						</div>
					}
				>
					...
					>
						...
						{locations.map((x) => (
							...
							>
								<div
									className="rounded-tl-full rounded-tr-full rounded-bl-full 
                 transform -translate-x-full -translate-y-full origin-bottom-right rotate-45"
								>
									<div
										className="border-4 bg-night rounded-tl-full rounded-tr-full rounded-bl-full 
                 transform m-0.5"
									>
										<div className=''>
											<img src={x.avatar} className="transform -rotate-45 hover:scale-110 transition ease-in-out h-12 w-12 rounded-full" alt="Food Truck Marker" />
										</div>
									</div>
								</div>
							</Marker>
		...
		<div className="absolute flex h-screen w-screen justify-center items-center">
			<h1 className="font-poppins font-bold mb-16 text-lg md:text-4xl">Fetching Your location...</h1>
		</div>
	);
