  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-screen justify-center">
      ...
      <div className = "flex-col">
        <div className="flex justify-center  py-4"><h2 className= "w-96 h-12 px-4 font-poppins font-semibold text-4xl">Register</h2></div>
        ...
        <button type="submit" className="w-96 h-12 bg-night px-4 rounded-xl focus:outline-none">
          <motion.p className="font-mulish font-bold text-white text-xl focus:outline-none" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>Register</motion.p>
        </button>
        </div>
        </div>
      </form>
      {Object.keys(errors).length > 0 && (
        ...
        <ul className="w-96 text-center">
            {Object.values(errors).map((value) => (
              <span key={value} className="flex justify-center py-1 font-poppins text-base">{value}</span>
            ))}
          </ul>
        </div>
      )}
      ...
      <div className= 'hidden md:flex h-2/4 sm:h-screen w-screen md:w-2/4 items-center bg-pink'>
        <h2 className="px-16 font-poppins font-black text-4xl">Welcome Back. <br />Sign  in to your Account</h2>
      </div>
